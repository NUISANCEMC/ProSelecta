...
TEST_CASE("project energy", "[ps::part]") {
  ...
  REQUIRE_THAT(energy(proton_at_rest), WithinAbs(0.938, 1E-8));
  REQUIRE_THAT(energy(fsnu), WithinAbs(1, 1E-8));
}
...
TEST_CASE("project momentum", "[ps::part]") {
  ...
  REQUIRE_THAT(momentum(proton).m(), WithinAbs(0.938, 1E-8));
  REQUIRE_THAT(momentum(proton).p3mod(), WithinAbs(1, 1E-8));
...
TEST_CASE("p3mod cuts", "[ps::part]") {
  ...
  REQUIRE_FALSE((p3mod < 0.5)(protons.back()));
  REQUIRE((p3mod <= 0.5)(protons.back()));
  REQUIRE_FALSE((p3mod > 0.5)(protons.back()));
  REQUIRE((p3mod >= 0.5)(protons.back()));

  REQUIRE((p3mod < 1)(protons.back()));
  REQUIRE((p3mod <= 1)(protons.back()));

  REQUIRE((p3mod > 0.25)(protons.back()));
  REQUIRE((p3mod >= 0.25)(protons.back()));

  REQUIRE(((p3mod < 1) && (p3mod > 0.25))(protons.back()));
}
...
TEST_CASE("filter p3mod", "[ps::part]") {
  ...
  REQUIRE(part::filter(p3mod > 2, protons).size() == 0);
  REQUIRE(part::filter(p3mod > 1, protons).size() == 1);
  REQUIRE(part::filter(p3mod >= 1, protons).size() == 2);
  REQUIRE(part::filter(p3mod > 0.25, protons).size() == 3);
  REQUIRE(part::filter(p3mod > 0.75 && p3mod < 1.25, protons).size() == 1);
}
